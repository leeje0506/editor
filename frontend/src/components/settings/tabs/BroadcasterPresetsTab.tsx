...
export function BroadcasterPresetsTab({ dark = true }: Props) {
  ...
  const loadRules = async () => {
    try {
      ...
      const list: Rule[] = Object.entries(data).map(([name, r]: [string, any]) => ({
        name, max_lines: r.max_lines, max_chars_per_line: r.max_chars_per_line,
        allow_overlap: r.allow_overlap ?? false, min_duration_ms: r.min_duration_ms ?? 500,
  ...
  const handleAdd = async () => {
    ...
    if (rules.some(r => r.name === newName.trim())) { setMsg("이미 존재하는 방송사입니다"); setTimeout(() => setMsg(""), 2000); return; }
    const updated = [...rules, { name: newName.trim(), max_lines: newLines, max_chars_per_line: newChars, allow_overlap: newOverlap, min_duration_ms: newMinDur, speaker_mode: newSpeakerMode }];
    setRules(updated); await saveToServer(updated); setNewName("");
  };

  const handleDelete = async (idx: number) => {
    if (!confirm(`"${rules[idx].name}" 방송사 기준을 삭제하시겠습니까?`)) return;
    const updated = rules.filter((_, i) => i !== idx); setRules(updated); await saveToServer(updated);
  };

  const startEdit = (idx: number) => { setEditIdx(idx); setEditRule({ ...rules[idx] }); };
  const cancelEdit = () => { setEditIdx(null); setEditRule(null); };
  const confirmEdit = async () => {
    if (editIdx === null || !editRule) return;
    const updated = rules.map((r, i) => i === editIdx ? editRule : r);
    setRules(updated); await saveToServer(updated); setEditIdx(null); setEditRule(null);
  };
    ...
    <div className="max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className={`text-lg font-bold ${tp}`}>방송사별 자막 기준</h2>
        </div>
        {msg && <span className={`text-xs font-medium ${msg.includes("실패") || msg.includes("이미") ? "text-red-400" : "text-emerald-400"}`}>{msg}</span>}
      </div>
      ...
      <div className={`${card} border ${bd} rounded-xl p-5 mb-6`}>
        <div className="flex items-end gap-3">
          <div className="flex-1">
            ...
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="예: NETFLIX" onKeyDown={e => { if (e.key === "Enter") handleAdd(); }}
          ...
          <div className="w-20">
            ...
            <button onClick={() => setNewOverlap(!newOverlap)}
          ...
          <div className="w-28">
            ...
            <select value={newSpeakerMode} onChange={e => setNewSpeakerMode(e.target.value)} className={`w-full border rounded-lg px-2 py-2.5 text-sm outline-none ${inp}`}>
              <option value="name">이름</option><option value="hyphen">하이픈</option><option value="hyphen_space">하이픈공백</option>
            </select>
          </div>
          <button onClick={handleAdd} disabled={saving} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white p-2.5 rounded-lg"><Plus size={20} /></button>
        </div>
      </div>

      <div className={`${card} border ${bd} rounded-xl divide-y ${divider}`}>
        ...
        {rules.map((r, i) => (
          <div key={i} className="flex items-center px-5 py-3">
            {editIdx === i && editRule ? (
              <>
                <input value={editRule.name} onChange={e => setEditRule({ ...editRule, name: e.target.value })} className={`flex-1 border rounded px-2 py-1 text-sm outline-none ${inp} focus:border-blue-500`} />
                <input type="number" value={editRule.max_lines} onChange={e => setEditRule({ ...editRule, max_lines: +e.target.value })} className={`w-20 border rounded px-2 py-1 text-sm text-center outline-none ${inp}`} />
                <input type="number" value={editRule.max_chars_per_line} onChange={e => setEditRule({ ...editRule, max_chars_per_line: +e.target.value })} className={`w-20 border rounded px-2 py-1 text-sm text-center outline-none ${inp}`} />
                <input type="number" step="0.1" value={editRule.min_duration_ms / 1000} onChange={e => setEditRule({ ...editRule, min_duration_ms: Math.round(parseFloat(e.target.value || "0") * 1000) })} className={`w-20 border rounded px-2 py-1 text-sm text-center outline-none ${inp}`} />
                <div className="w-20 flex items-center justify-center">
                  <button onClick={() => setEditRule({ ...editRule, allow_overlap: !editRule.allow_overlap })} className={`px-2 py-1 rounded text-xs ${editRule.allow_overlap ? "bg-emerald-600 text-white" : dm ? "bg-gray-700 text-gray-400" : "bg-gray-200 text-gray-600"}`}>
                    {editRule.allow_overlap ? "허용" : "미허용"}
                  </button>
                </div>
                <div className="w-24 flex items-center justify-center">
                  <select value={editRule.speaker_mode} onChange={e => setEditRule({ ...editRule, speaker_mode: e.target.value })} className={`px-1 py-1 rounded text-xs border outline-none ${inp}`}>
                    <option value="name">이름</option><option value="hyphen">하이픈</option><option value="hyphen_space">하이픈공백</option>
                  </select>
                </div>
                <div className="w-20 flex items-center justify-center gap-1">
                  <button onClick={confirmEdit} className="text-emerald-400 hover:text-emerald-300"><Check size={16} /></button>
                  <button onClick={cancelEdit} className={`${ts} hover:opacity-70`}><X size={16} /></button>
                </div>
              </>
            ) : (
              <>
                <span className={`flex-1 font-medium text-sm ${tp}`}>{r.name}</span>
                ...
                <span className={`w-20 text-center text-xs ${r.allow_overlap ? "text-emerald-400" : "text-red-400"}`}>{r.allow_overlap ? "허용" : "미허용"}</span>
                <span className={`w-24 text-center text-xs ${ts}`}>{speakerModeLabel(r.speaker_mode)}</span>
        ...
        {rules.length === 0 && <div className={`px-5 py-8 text-center text-sm ${ts}`}>등록된 방송사 기준이 없습니다.</div>}
      </div>
    </div>
  );
}