...
export function GridToolbar({ dark, filteredCount, totalCount }: Props) {
  const [showBulk, setShowBulk] = useState(false);
  const [showKeys, setShowKeys] = useState(false);
  const { multiSelect, addAfter, deleteSelected, splitSelected, mergeSelected } = useSubtitleStore();
  ...
  return (
    <>
      <div className={`h-9 px-3 flex items-center justify-between border-b ${bdl}`}>
        <div className="flex items-center gap-2">
          <span className={`text-xs font-bold ${tp}`}>
            자막 리스트 <span className={`${ts} font-normal`}>(Total: {filteredCount} / {totalCount})</span>
          </span>
          <button onClick={() => setShowKeys(true)} className={`flex items-center gap-0.5 border ${bd} ${dm ? "bg-gray-700" : "bg-gray-50"} ${ts} px-1.5 py-0.5 rounded text-[9px]`}>
      ...
      {showBulk && <BulkSpeakerModal dark={dm} onClose={() => setShowBulk(false)} />}
