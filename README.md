...
> **설계 의도**: 마스터와 관리자는 "관리자급"으로 대시보드, 설정, 통계 등에 접근 가능. 작업자는 편집기와 자기 프로젝트에만 접근 가능. 이렇게 분리하여 권한 범위를 명확히 하고, 작업자가 다른 사람의 프로젝트를 수정하거나 설정을 변경하는 것을 방지.
...
> **설계 의도**: `/`에 접속하면 `RootRedirect`가 localStorage의 user 정보를 읽어서 역할에 따라 분기. master/manager는 `/dashboard` (대시보드 요약 + 프로젝트 리스트), worker는 `/projects` (프로젝트 리스트만). 같은 `HomePage` 컴포넌트를 재사용하되, URL에 따라 대시보드 섹션 표시 여부를 분기.
...
> **의도**: JWT 토큰 기반 인증. 서버 시작 시 master 계정이 없으면 `admin/admin`으로 자동 생성. 모든 API에 인증 필수 (health 제외).
...
### B01-T05. 초기 시딩 (`app/main.py` startup 이벤트)
...
> **의도**: 프로젝트는 자막 작업의 단위. 영상+자막 파일을 업로드하여 생성하고, 작업자가 편집한 뒤 제출. 관리자가 검수 후 승인/반려.
...
- `status` (draft/submitted/approved), `elapsed_seconds`, `last_saved_at`, `submitted_at`, `deadline` — 상태/시간
...
```
- `POST /api/projects/:id/submit` — 검수 오류 있으면 400 거부. 없으면 status=submitted. `first_submitted_at`은 최초 제출 시에만 기록 (재제출 시 변경 안 함)
...
> **설계 의도 — 제출 이후 동작:**
> - **관리자(master/manager)**: 제출된 작업을 열어 읽기전용으로 검수 가능. 각 자막을 확인하고 승인/반려 판정.
>   - 승인 → status=approved. 확정.
>   - 반려(미승인) → status=rejected. 작업자가 다시 수정할 수 있도록 돌려줌.
> - **작업자(worker)**: 제출 후 작업을 열 수 있지만 **확인만 가능** (읽기전용, 검수 기능 없음).
> - **반려 후 작업자**: rejected 상태에서 편집 가능 (재작업). 재제출하면 동일 과정 반복.
> - **반려 후 관리자**: 진행 중 탭에서 "재작업" 뱃지로 표시. 읽기전용.

### B02-T04. 재작업 추적
- `reject_count` (int, default 0) — 반려 횟수. 반려할 때마다 +1
- `first_submitted_at` (datetime, nullable) — 최초 제출 일시. 처음 제출할 때만 기록, 재제출 시 변경 안 함

> **설계 의도**: 같은 사람이 재작업을 2회 이상 하더라도 `first_submitted_at`은 항상 최초 제출 시점. 관리자가 작업자의 최초 완료일과 재작업 횟수를 함께 볼 수 있어 작업 품질 관리에 활용.

### B02-T05. 프로젝트 접근 권한
- **목록 조회**: worker는 `assigned_to == me OR created_by == me`인 프로젝트만 조회 가능
- **상세/편집기 접근**: worker는 본인 배정 또는 본인 생성 프로젝트만 접근 가능. 아닌 경우 403
- **담당자 변경**: 관리자가 다른 작업자로 변경하면, 기존 작업자(created_by가 아닌 경우)는 목록에서 사라지고 접근 불가
...
> **의도**: 방송사마다 자막 기준이 다름 (최대 줄 수, 줄당 글자 수, 화자 예약 글자). 이를 DB에 저장하여 설정 페이지에서 관리자가 추가/수정/삭제. 프로젝트 생성 시 방송사 선택하면 해당 기준이 자동 적용.
...
- `PUT /api/settings/broadcaster-rules` — 전체 덮어쓰기. 기존 전부 비활성화 → 요청 데이터로 추가/수정
- `GET /api/projects/rules/broadcasters` — `load_rules()` 호출 (프로젝트 생성 시 사용)

### B03-T03. `load_rules()` 함수
- DB에서 활성 규칙 로드. 다른 모듈(projects.py)에서도 import하여 사용
- DB가 비어있으면 기본값 시딩 (TVING, LGHV, SKBB, JTBC, DLIV, 자유작업)

### B03-T04. 전역 스토어 연동 (`useBroadcasterStore`)
- 프론트에서 Zustand 전역 스토어로 방송사 규칙 관리
- 페이지 진입 시 `fetch()` 호출 → API에서 최신 데이터 로드
- 설정에서 저장 후 `fetch()` 재호출 → 모든 페이지에 즉시 반영

---

## ACT-B04. 파일 관리

> **의도**: SRT 파일을 업로드하면 서버에서 파싱하여 개별 자막 레코드로 DB에 저장. 태그(`{\an8}`, `{\}`, `{\ㅅ}`)를 파싱하여 메타데이터로 분리. 다운로드 시 역순으로 SRT 형식 복원.

### B04-T01. SRT 파싱 (`parse_srt` in `app/services/subtitle_service.py`)
- 타임코드: `HH:MM:SS,mmm --> HH:MM:SS,mmm` → `start_ms`, `end_ms`
- 태그: `{\an8}` → `text_pos = "top"` (상단이동). `{\}` → 유지 표시 (제거). `{\ㅅ}` → 삭제 표시 (제거)
- 화자: 텍스트 시작부 `(이름)` 패턴 → `speaker` 필드 분리
- 유형: 전체가 `[효과음]` 패턴 → `type = "effect"`, 나머지 → `type = "dialogue"`

### B04-T02. SRT 생성 (`export_srt`)
- 출력: `순번\n타임코드\n태그+화자+대사\n\n`
- 상단이동: `{\an8}` 접두. 화자: `(화자명) `. 대사 텍스트 그대로

### B04-T03. 파일 업로드
- `POST /api/projects/:id/upload/subtitle` — SRT 업로드 → 파싱 → DB 저장. 기존 자막 전체 삭제 후 재삽입
- `POST /api/projects/:id/upload/video` — 영상 파일 1MB 청크 저장. `uploads/project_{id}_{filename}`

### B04-T04. 다운로드 / 스트리밍
- `GET /api/projects/:id/download/subtitle` — SRT 텍스트 응답 (Content-Disposition 헤더)
- `GET /api/projects/:id/stream/video` — `FileResponse`로 영상 스트리밍 (seek 지원)

---

## ACT-B05. 자막 CRUD

> **의도**: 자막은 프로젝트 내 개별 싱크(sync) 단위. 순번(seq)으로 정렬. 모든 변경 후 순번 재계산 + 검수 자동 실행.

### B05-T01. Subtitle 모델 (`app/models.py`)
- `id`, `project_id`, `seq`, `start_ms`, `end_ms`, `type` (dialogue/effect), `speaker`, `speaker_pos` (default/top), `text_pos` (default/top), `text`, `error`

### B05-T02. 자막 API (`app/routers/subtitles.py`)
- `GET /api/projects/:pid/subtitles` — 전체 목록 (seq 순)
- `POST /api/projects/:pid/subtitles` — 싱크 추가. `after_seq` 기준, `start_ms = 이전 end_ms + 10`, 2초 길이
- `PATCH /api/projects/:pid/subtitles/:id` — 단건 수정
- `DELETE /api/projects/:pid/subtitles/:id` — 단건 삭제
- `POST .../subtitles/batch-delete` — 다중 삭제
- `POST .../subtitles/:id/split` — 분할 (시간 절반 + 대사 공백 기준 스마트 분할)
- `POST .../subtitles/merge` — 병합 (시간 합치기 + 텍스트 줄바꿈 연결)
- `POST .../subtitles/bulk-speaker` — 화자 일괄 변경
- `PUT .../subtitles/batch-update` — 전체 저장 (임시저장)
- `POST .../subtitles/undo` — 되돌리기 (최근 스냅샷 복원)

### B05-T03. 검수 (`resequence_and_validate`)
- 모든 자막 변경 API 실행 후 자동 호출
- 순번 재계산: `start_ms ASC, id ASC` 순서로 seq 1부터 부여
- 검수 규칙:

| 규칙 | 조건 | error 값 |
|------|------|----------|
| 글자 초과 (화자 있음) | 줄 글자수 > `max_chars_per_line - bracket_chars` | `"글자초과"` |
| 글자 초과 (화자 없음) | 줄 글자수 > `max_chars_per_line` | `"글자초과"` |
| 줄 수 초과 | 줄 수 > `max_lines` | `"줄초과"` |
| 시간 오류 | `end_ms ≤ start_ms` | `"시간오류"` |

> **글자수 카운트 의도**: 공백 포함, 줄바꿈만 제외. NFD→NFC 정규화 후 카운트. 효과음 `[텍스트]`의 대괄호도 포함. 화자명은 별도 필드이므로 카운트 대상 아님.

### B05-T04. Undo (스냅샷 기반)
- `save_snapshot()`: 변경 전 전체 자막 상태를 JSON으로 `EditHistory` 테이블에 저장
- `restore_snapshot()`: 최근 스냅샷 → 현재 자막 전체 삭제 → 스냅샷 데이터로 재생성
...
### F01-T01. 로그인 페이지 (`/login`)
- 아이디/비밀번호 입력 → `POST /api/auth/login` → JWT 토큰 + User 정보를 `localStorage` 저장
- 실패 시 빨간 에러 메시지. 성공 시 `RootRedirect`로 이동

### F01-T02. useAuthStore (Zustand)
- `user`, `token`, `isAuthenticated`, `login()`, `logout()`, `isAdmin()`, `loadUser()`
- `isAdmin()`: `user.role === "master" || user.role === "manager"`
- `logout()`: localStorage 클리어 + `/login`으로 이동

### F01-T03. ProtectedRoute 컴포넌트
- 미인증 → `/login` 리다이렉트
- `roles` prop 있으면 역할 체크 → 불일치 시 `/projects`로 리다이렉트

### F01-T04. Axios 인터셉터 (`api/client.ts`)
...
> **의도**: master/manager는 `/dashboard`에서 대시보드 요약(상태별 카드, 차트, 검수 대기 목록) + 프로젝트 리스트를 볼 수 있음. worker는 `/projects`에서 프로젝트 리스트만 봄. 같은 컴포넌트를 재사용하되 URL로 분기.

### F02-T01. 헤더
- 로고 "SubEditor Pro" + 버전
- 사용자명 + 역할 뱃지 (master=빨강, manager=보라, worker=파랑)
- ⚙ 설정 버튼 → `navigate("/settings")`
- ☀/🌙 다크모드 토글 — `const [dark, setDark] = useState(true)`. 전체 컴포넌트 스타일 변수가 `dm`에 따라 분기
- 로그아웃 버튼

### F02-T02. 사이드바
- "작업" 섹션: 새 작업 시작하기 → `NewProjectModal` 오픈
- "보기" 섹션 (admin만): 대시보드 버튼 → `/dashboard`
- "도구" 섹션: 자막 버전 비교, 완성본 검수 (admin만)
- "도움말" 섹션: 사용자 가이드

### F02-T03. 대시보드 요약 (admin + `/dashboard`일 때만)
- 4개 상태 카드: 대기 중 / 진행 중 / 검토 대기 / 승인 완료 — 각각 건수 표시
- 2개 차트: 작업자별 진행 건수 (바 차트), 방송사별 진행 현황 (바 차트)
- 검수 대기 목록: `status === "submitted"` 프로젝트 리스트

### F02-T04. 프로젝트 리스트
- 탭: 진행 중 / 제출됨 / 승인됨 — 각 탭에 건수 뱃지
  - "진행 중" 탭에는 `draft` + `rejected` 프로젝트 모두 포함
- 검색: 프로젝트명 키워드 실시간 필터
- 방송사 필터: `useBroadcasterStore.names`에서 전체 방송사 목록 드롭다운. 선택 시 해당 방송사 프로젝트만 표시
- 각 프로젝트 행:
  - 상태 뱃지 (진행중=파랑, 제출됨=노랑, 승인됨=초록, 반려됨=주황)
  - D-day 뱃지 (마감 7일 이내 표시, 3일 이내 빨강)
  - 재작업 뱃지 — `rejected` 상태이거나 `reject_count > 0`인 경우 "재작업 (N회)" 주황 뱃지 표시
  - 프로젝트명 (인라인 수정 가능)
  - 방송사 · 설명
  - 영상 길이 / 작업 시간 / 용량
  - 담당자 / 마감일 (작업자 변경 가능 — 등록된 사용자 드롭다운)
    - `reject_count > 0`인 경우: 마감일 아래에 "기존 완료: {first_submitted_at}" + "재작업: N회" 표시
  - 액션 버튼:
    - draft → "작업 열기"
    - submitted + 관리자 → "검수 열기"
    - submitted + 작업자 → "확인" (읽기전용, 검수 기능 없음)
    - approved → "확인"
    - rejected → "재작업" (작업자에게만, 편집 가능)
    - 다운로드 / ⋮ 메뉴

### F02-T05. ⋮ 메뉴 (createPortal로 document.body에 렌더링)
- 이름 수정: 인라인 input으로 전환 → Enter로 저장 → `projectsApi.update(id, { name })`
- 작업자 변경 (admin만): `authApi.listUsers()`에서 전체 사용자 select → `projectsApi.update(id, { assigned_to })`
- 삭제: confirm → `projectsApi.delete(id)`

> **의도**: ⋮ 메뉴는 `createPortal`로 `document.body`에 렌더링. 부모의 `overflow-hidden`에 잘리는 문제 해결. 버튼의 `getBoundingClientRect()`로 좌표 계산, 버튼 왼쪽에 배치하여 화면 오른쪽 잘림 방지.
...
### F03-T01. 모달 필드
- 프로젝트 명칭 (필수)
- 방송사 (필수) — `useBroadcasterStore.names`에서 동적 목록. 선택 시 자막 기준 읽기전용 표시
- 설명/부제
- 마감일 — `<input type="date">` + `showPicker()` 강제 호출 + `colorScheme` 다크모드 대응 + `min={오늘}` 과거 날짜 방지
- 영상 파일 (필수) — `accept="video/*"`
- 자막 파일 (필수) — `accept=".srt,.vtt,.txt"`

### F03-T02. 생성 흐름
1. 유효성 검사 (이름, 영상, 자막 필수)
2. `POST /api/projects` → 프로젝트 생성 (방송사 기준 자동 적용)
3. `POST /api/projects/:id/upload/video` → 영상 업로드
4. `POST /api/projects/:id/upload/subtitle` → 자막 업로드 + 파싱
5. `GET /api/projects/:id` → 최종 정보 로드
6. `navigate("/editor/:id")` → 편집기로 이동

---

## ACT-F04. 편집기 레이아웃 (`AppLayout.tsx`)

> **의도**: Subtitle Edit의 레이아웃을 참고하되, 4개 패널(자막 리스트, 영상 플레이어, 퀵 에디터, 타임라인/파형)을 독립적으로 크기 조절 가능하게 배치.
...
- 리사이즈 핸들은 `HResizeHandle` 공용 컴포넌트. hover 시 파란 하이라이트 + 중앙 점선 표시
- 드래그 중 `document.body.style.userSelect = "none"` + 전역 오버레이로 안정적 드래그
...
- `readOnly`를 TopNav, SubtitleGrid, QuickEditor에 전달
- 읽기전용: 편집 버튼 숨김, textarea disabled, 위치 토글 disabled, "읽기전용" 뱃지 표시
- 재생/탐색/선택은 가능 (검수를 위해)

### F04-T05. 작업 시간 추적
- `setInterval(1초)` → `elapsed + 1` (로컬 카운터)
- `setInterval(30초)` → `POST /api/projects/:id/timer` 서버 동기화
- 임시저장 시: `updateTimer` + `markSaved` 동시 호출
- 홈 이동 시: `updateTimer` 호출 후 navigate
